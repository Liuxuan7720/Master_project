...
def read_3dgs_ply(p):
    ...
    if rest:
        ...
        fr   = np.stack([v[n] for n in rest], 1).astype(np.float32)   # (N, 3*C)
        C    = fr.shape[1] // 3
        L    = int(round(math.sqrt(C + 1) - 1))
        deg  = max(0, L)
        f_rest = fr.reshape(-1, 3, C)

    rfs = [f for f in v.dtype.names if f.startswith("rot_")] or \
...
def eval_sh_basis(deg, d):
    ...
    if deg >= 3:
        ...
        out += [
            c3[0]*y*(3*x*x-y*y), -c3[1]*x*y*z, c3[2]*y*(5*z*z-1.0),
            -c3[3]*z*(5*z*z-3.0), c3[2]*x*(5*z*z-1.0),
            -c3[1]*z*(x*x-y*y),  c3[4]*x*(x*x-3*y*y)
        ]
...
def sh_color_topdown(fdc, f_rest, deg, Rquat, n):
    if deg <= 0 or f_rest is None or Rquat is None:
        return np.clip(1.0 / (1.0 + np.exp(-(fdc * Y00))), 0, 1)
    Rm = quat_to_R(Rquat)
    v = (-n).astype(np.float32); v /= (np.linalg.norm(v)+1e-12)
    dirs = (Rm.transpose(0,2,1) @ v[None,:].repeat(Rm.shape[0],0)[...,None]).squeeze(-1)
    dirs /= (np.linalg.norm(dirs, axis=1, keepdims=True) + 1e-12)
    B = eval_sh_basis(deg, dirs)
    feats = np.concatenate([fdc[:,:,None], f_rest], 2)
    rgb = 1.0 / (1.0 + np.exp(-(feats * B[:,None,:]).sum(2)))
...
# ---------- 超采样软分配 + 平滑 ----------
def softsplat_ss(X, rgb, n, p0, u, v, opa=None,
                 pix=0.010, pad=0.002, ss=3, beta=4.5,
                 q_low=0.30, q_high=0.70, inpaint_tau=0.03):
    ...
    for wij, jj, ii in [(w00,j,i),(w10,j+1,i),(w01,j,i+1),(w11,j+1,i+1)]:
        ...
        f=flat(ii,jj)
        w=(wij*wdepth*wopa).astype(np.float32)
        np.add.at(den,f,w)
        np.add.at(acc,f,(rgb*w[:,None]).astype(np.float32))

    img_hi   = acc.reshape(H_hi, W_hi, 3)
    Wsum_hi2 = den.reshape(H_hi, W_hi).astype(np.float32)  # 2D

    # 平滑分子/分母，再相除
    sigma = 0.6*ss
    img_hi  = cv2.GaussianBlur(img_hi,  (0,0), sigmaX=sigma, sigmaY=sigma, borderType=cv2.BORDER_REPLICATE)
    Wsum_hi = cv2.GaussianBlur(Wsum_hi2,(0,0), sigmaX=sigma, sigmaY=sigma, borderType=cv2.BORDER_REPLICATE)
    Wsum_hi = Wsum_hi[..., None]  # 扩一维，(H,W,1)

    img_hi = np.divide(img_hi, np.clip(Wsum_hi, 1e-6, None))

    # 下采样（块平均）
    img  = img_hi.reshape(Ht, ss, W, ss, 3).mean(3).mean(1)
    wsum = Wsum_hi.reshape(Ht, ss, W, ss, 1).mean(3).mean(1)
...
def main():
    ...
    ap.add_argument("--gain", type=float, default=1.08)
    ap.add_argument("--sat_boost", type=float, default=1.15)
    ap.add_argument("--out", default="ortho_soft_v3.png")
    args=ap.parse_args()
    ...
    img = softsplat_ss(X, rgb, n, p0, u, v, opa=opa,
