# ml_point_filter_v4_ceiling.py
# Purpose: Point-level filtering based on geometry/statistics + light ML to keep the main body and remove far background/clouds;
#          additionally, perform “ceiling peeling” within the main ROI (keep only the base layer heights) to suppress clouds above the subject.
...
def read_ply(p):
    ...
    # f_dc_* kept only for interface consistency; color not used in this script
...
def local_height_stats(h, grid, ksize=3):
    ...
    for (y,x), idxs in buckets.items():
        med[y,x] = np.median(h[idxs])
    # Fill empty cells (neighbor average)
    mask = np.isnan(med).astype(np.uint8)
    if mask.any():
        med_f = med.copy(); med_f[mask==1] = 0
        w = (~mask.astype(bool)).astype(np.float32)
        ker = np.ones((ksize,ksize), np.float32)
        num = cv2.filter2D(med_f, -1, ker, borderType=cv2.BORDER_REPLICATE)
    ...
    if mask2.any():
        ...
        ker = np.ones((ksize,ksize), np.float32)
        num2 = cv2.filter2D(MAD_f,-1,ker,borderType=cv2.BORDER_REPLICATE)
        den2 = cv2.filter2D(w2,-1,ker,borderType=cv2.BORDER_REPLICATE)+1e-6
        MAD = num2/den2
    mad_cell = MAD[vi,ui]
...
def scale_stats(scales):
    if scales is None:
        N = 1
        return np.zeros(N,np.float32),np.zeros(N,np.float32),np.ones(N,np.float32)
    s_mean = scales.mean(1); s_max = scales.max(1)
    aniso  = (s_max+1e-6)/(scales.min(1)+1e-6)
...
def build_features(X, fdc, opa, scales, n, p0):
    ...
    parts = [h[:,None], r[:,None], den[:,None], dev[:,None], s_mean[:,None], s_max[:,None], aniso[:,None]]
    if opa is not None: parts.append(opa[:,None])
    F = np.concatenate(parts,1).astype(np.float32)
    mu = F.mean(0, keepdims=True); sd = F.std(0, keepdims=True)+1e-6
    Fz = (F-mu)/sd
    aux = {"h":h, "den":den, "dev":dev, "opa":opa, "U":U, "V":V, "s_mean":s_mean}
    return Fz, aux

# ----------------- Weak labels -----------------
def weak_labels_keep(aux, q_pos_dev=1.5, q_neg_dev=3.5, q_hi_h=0.98, q_den_pos=0.60, q_den_neg=0.10, opa_min=0.03):
    h, den, dev, opa = aux["h"], aux["den"], aux["dev"], aux["opa"]
    N = len(h); y = np.full(N, -1, np.int64)
    # Keep-main-body head: locally consistent height & high density & sufficiently opaque
    pos = (dev <= q_pos_dev) & (den >= np.quantile(den, q_den_pos))
    if opa is not None: pos &= (opa >= max(opa_min, np.quantile(opa,0.20)))
    # Negative: strong local outliers OR (very high height + very low density) OR very low opacity
    neg = (dev >= q_neg_dev) | ((h >= np.quantile(h, q_hi_h)) & (den <= np.quantile(den, q_den_neg)))
    if opa is not None: neg |= (opa < opa_min)
    y[pos] = 1; y[neg] = 0
    return y

def weak_labels_cloud(aux, q_hi_h=0.97, q_den_low=0.20, dev_hi=3.0, s_mean_q=0.85, opa_soft=0.06):
    h, den, dev, opa, s_mean = aux["h"], aux["den"], aux["dev"], aux["opa"], aux["s_mean"]
    N = len(h); y = np.full(N, -1, np.int64)
    # Cloud head: high height with low density or high deviation; optionally large average scale or soft opacity
    h_thr  = np.quantile(h, q_hi_h)
    den_lo = np.quantile(den, q_den_low)
    s_big  = np.quantile(s_mean, s_mean_q) if not np.isscalar(s_mean) else 0.0
    pos = (h >= h_thr) & ((den <= den_lo) | (dev >= dev_hi))
    if opa is not None: pos &= (opa <= opa_soft) | (s_mean >= s_big)
    else: pos &= (s_mean >= s_big)
    # Negative examples for clouds: mid/low height, decent density, small deviation
    neg = (h <= np.quantile(h, 0.85)) & (den >= np.quantile(den, 0.50)) & (dev <= 1.5)
    y[pos] = 1; y[neg] = 0
    return y
...
def train(F, y, epochs=10, device="cpu"):
    idx = np.where(y!=-1)[0]
    if len(idx) < 1000:
        return np.ones(len(F), np.float32)*0.5
...
def roi_lock(U,V,dev,P,thr=0.55, cell=0.14):
    ...
    if num>1:
        ...
        core=keep
    # Expand by including points with small height deviation to prevent fragmentation
    near = (dev <= 2.5)
    final = core | near
    # Remove isolated sparse regions via morphology and largest-component selection
...
# ----------------- Ceiling peel -----------------
def ceiling_peel(U, V, h, core_mask, cell=0.18, p=0.35, tolq=0.25):
    """
    Perform “ceiling peeling” inside the main ROI: per cell, use the p-quantile of height as base, add tolerance, and keep only the base layer.
    """
    idx = np.where(core_mask)[0]
    keep_all = np.ones_like(h, dtype=bool)
    if idx.size == 0:  # No ROI → no action
        return keep_all

    Uc, Vc, hc = U[idx], V[idx], h[idx]
    u0, v0 = Uc.min(), Vc.min()
    ui = ((Uc - u0) / cell).astype(np.int32)
    vi = ((Vc - v0) / cell).astype(np.int32)
    H, W = vi.max() + 1, ui.max() + 1

    qmap = np.full((H, W), np.nan, np.float32)
    buckets = {}
    for k, (y, x) in enumerate(zip(vi, ui)):
        buckets.setdefault((y, x), []).append(k)
    for (y, x), kk in buckets.items():
        qmap[y, x] = np.quantile(hc[kk], p)

    mask = np.isnan(qmap).astype(np.uint8)
    if mask.any():
        qf = qmap.copy(); qf[mask == 1] = 0
        w  = (~mask.astype(bool)).astype(np.float32)
        ker = np.ones((3,3), np.float32)
        num = cv2.filter2D(qf, -1, ker, borderType=cv2.BORDER_REPLICATE)
        den = cv2.filter2D(w,  -1, ker, borderType=cv2.BORDER_REPLICATE) + 1e-6
        qmap = num / den

    iqr = np.quantile(hc, 0.75) - np.quantile(hc, 0.25)
    tol = max(1e-3, float(iqr) * float(tolq))

    qp = qmap[vi, ui] + tol
    keep_roi = hc <= qp

    keep_all[idx] = keep_roi
    return keep_all
...
def main():
    ...
    ap.add_argument("--out_ply", default="filtered_v4.ply")
    ap.add_argument("--epochs", type=int, default=12)
    ap.add_argument("--device", default="cpu")
    ap.add_argument("--keep_thr",  type=float, default=0.55)
    ap.add_argument("--cloud_thr", type=float, default=0.55)
    ap.add_argument("--opa_min",   type=float, default=0.03)
    # Ceiling-peel controls
    ap.add_argument("--ceil_on",   type=int,   default=1)
    ap.add_argument("--ceil_cell", type=float, default=0.20)
    ap.add_argument("--ceil_p",    type=float, default=0.35)
    ap.add_argument("--ceil_tolq", type=float, default=0.25)
    args = ap.parse_args()
    ...
    # 1) Keep-main-body head
    y_keep = weak_labels_keep(aux, opa_min=args.opa_min)
    P_keep = train(F, y_keep, epochs=args.epochs, device=args.device)
    keep0  = roi_lock(U,V, aux["dev"], P_keep, thr=args.keep_thr, cell=0.14)

    # 2) Cloud-detection head (trained globally; applied after protecting the main body)
    y_cloud = weak_labels_cloud(aux)
    P_cloud = train(F, y_cloud, epochs=max(6,args.epochs//2), device=args.device)
    h, den = aux["h"], aux["den"]
    guard = (h <= np.quantile(h, 0.90)) & (den >= np.quantile(den, 0.40))
    cloud_mask = (P_cloud >= args.cloud_thr) & (~guard)

    keep = keep0 & (~cloud_mask)

    # 3) Ceiling peeling inside the main ROI
    if args.ceil_on:
        peel_mask = ceiling_peel(U, V, h, keep0, cell=args.ceil_cell,
                                 p=args.ceil_p, tolq=args.ceil_tolq)
        keep = keep & peel_mask

    write_filtered_ply(v, keep, args.out_ply)
    print(f"[done] keep {keep.sum()}/{len(keep)} points → {args.out_ply}")
